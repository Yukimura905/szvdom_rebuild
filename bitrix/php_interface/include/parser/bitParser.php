...
class bitParser {
    ...
    function getCurentObjecktsList(){
		...
		while($ob = $res->GetNextElement()) {
			...
			$this->arCurObjects[ $arProps['SER_XML_ID']['VALUE'] ]['UPDATE_AUTO'] = 'Да';$arProps['SER_UPDATE_AUTO']['VALUE'];
	...
	function insertObjekts(){
		$arObjekts = $this->makeArray();//return;
	...
	function toArrayApartments($array) {
		...
		$roomIDsEnabled = array(0,1,2,3,4,5,22,23,21,28,29,6,25,8,7,32,30);
		...
		foreach($arRoomsType as $id=>$name){
			if( !in_array($id, $roomIDsEnabled) ) continue;
		...
		foreach ($array as $item) {
			...
			if( !in_array($roomID, $roomIDsEnabled) ) continue;
